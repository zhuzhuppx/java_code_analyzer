...
<div class="stats-grid">
  <div class="stat-card"><div class="stat-value">20</div><div class="stat-label">Java 类</div></div>
  <div class="stat-card"><div class="stat-value">4429</div><div class="stat-label">总行数</div></div>
  <div class="stat-card"><div class="stat-value">276</div><div class="stat-label">方法数</div></div>
  <div class="stat-card"><div class="stat-value">131</div><div class="stat-label">字段数</div></div>
  ...
  <div class="stat-card"><div class="stat-value">6</div><div class="stat-label">上帝类</div></div>
...
| 3 | 💩 上帝类 | com.projectassistant.model.ProjectModel 有 21 个方法 | 建议拆分为多个职责单一的类 |
| 4 | 💩 上帝类 | com.projectassistant.spring.SpringScanner 有 28 个方法 | 建议拆分为多个职责单一的类 |
...
| 2 | 💩 大类别 | com.projectassistant.knowledge.KnowledgeBaseGenerator 有 676 行 | 建议拆分为多个小类 |
...
| 7 | 低内聚 | KnowledgeBaseGenerator 内聚度=0.00（仅 0/18 方法访问字段） | 方法不操作类字段，考虑是否应该放在其它类中 |
...
| 14 | 低内聚 | ProjectModel 内聚度=0.00（仅 0/21 方法访问字段） | 方法不操作类字段，考虑是否应该放在其它类中 |
...
| 19 | 低内聚 | BeanInfo 内聚度=0.06（仅 1/17 方法访问字段） | 方法不操作类字段，考虑是否应该放在其它类中 |
| 20 | 低内聚 | SpringScanner 内聚度=0.00（仅 0/28 方法访问字段） | 方法不操作类字段，考虑是否应该放在其它类中 |
| 21 | 低内聚 | SqlParser 内聚度=0.00（仅 0/12 方法访问字段） | 方法不操作类字段，考虑是否应该放在其它类中 |
| 22 | 低内聚 | TableInfo 内聚度=0.00（仅 0/16 方法访问字段） | 方法不操作类字段，考虑是否应该放在其它类中 |
...
| 13 | 可能的未使用方法 | BeanInfo.addInjectedBy() 在项目内部未被调用 | 考虑删除或标记为 @Deprecated |
| 14 | 可能的未使用方法 | SpringScanner.scan() 在项目内部未被调用 | 考虑删除或标记为 @Deprecated |
| 15 | 可能的未使用方法 | SqlParser.scan() 在项目内部未被调用 | 考虑删除或标记为 @Deprecated |
| 16 | 可能的未使用方法 | SqlParser.hasMyBatis() 在项目内部未被调用 | 考虑删除或标记为 @Deprecated |
| 17 | 可能的未使用方法 | SqlParser.hasJPA() 在项目内部未被调用 | 考虑删除或标记为 @Deprecated |
| 18 | 待办事项 | 项目中共有 20 处 TODO/FIXME | 定期清理待办事项，避免技术债务累积 |

### 信息提示 (1)

| # | 分类 | 描述 | 建议 |
|---|---|---|---|
| 1 | Public API 规模 | 项目共有 187 个 public 方法 | public API 是项目的对外契约，变更需谨慎 |
